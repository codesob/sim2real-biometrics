...
def plot_training_history(history):
    ...
    ax1.plot(epochs, history['loss'], 'r-o', label="Proposed Loss")
    ax1.set_title("Proposed Model: Loss Convergence")
    ax1.grid(True, alpha=0.3)
    ax1.legend()

    ax2.plot(epochs, history['train_acc'], 'b--', label="Train (Robust)")
    ax2.plot(epochs, history['val_acc'], 'g-x', label="Val (Clean)")
    ax2.set_title("Proposed Model: Accuracy (%)")
    ax2.grid(True, alpha=0.3)
    ax2.legend()
    
    plt.savefig(os.path.join(SAVE_DIR, "scratch_dynamics.png"), dpi=300)
...
def main():
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomRotation(15), # Added: Handles real-world head tilts
        transforms.RandomPerspective(distortion_scale=0.2, p=0.5),
        transforms.RandomApply([
            transforms.GaussianBlur(kernel_size=(3, 3), sigma=(0.1, 2.0)),
            transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3)
        ], p=0.6),
        transforms.RandomGrayscale(p=0.1), # Added: Forces model to learn shapes, not just color
        transforms.ToTensor(),
    ...
    ], momentum=0.9, weight_decay=1e-3) # Increased weight decay for better generalization
    
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=EPOCHS, eta_min=1e-5)
    
    # Label Smoothing = 0.1 is the key "Proposed" fix for Sim2Real
    criterion = torch.nn.CrossEntropyLoss(label_smoothing=0.1)
    
    os.makedirs(SAVE_DIR, exist_ok=True)
    best_acc, history = 0.0, {'loss': [], 'train_acc': [], 'val_acc': []}

    for epoch in range(EPOCHS):
        backbone.train()
        t_loss, t_correct, t_total = 0.0, 0, 0
        loop = tqdm(train_loader, desc=f"Proposed Epoch {epoch+1}/20")
        ...
        for images, labels in loop:
            ...
            # Using raw features for accuracy check
            with torch.no_grad():
        ...
        print(f"Stats: Loss={history['loss'][-1]:.4f}, Train={history['train_acc'][-1]:.2f}%, Val={val_acc:.2f}%")
        
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(backbone.state_dict(), os.path.join(SAVE_DIR, "proposed_best.pth"))
